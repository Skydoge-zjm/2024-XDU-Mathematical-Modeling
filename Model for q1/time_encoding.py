...
def train_set_generate():
    ...
    for month in month_train:
        root_path = f'./data/q1/data-2019-{month}.xlsx'
        df = pd.read_excel(root_path)
        df.rename(columns={'Date/Time': 'DateTime', 'Line Current [A] DEV1': 'LC'}, inplace=True)
    ...
    path = os.path.join('.', 'data', 'q1', filename)
...
def test_set_generate():
    ...
    dfs = []
    for month in month_train:
        root_path = f'./data/q1/data-2020-{month}.xlsx'
        df = pd.read_excel(root_path)
        df.rename(columns={'Date/Time': 'DateTime', 'Line Current [A] DEV1': 'LC'}, inplace=True)
        df['DateTime'] = pd.to_datetime(df['DateTime'])
        df = df.set_index('DateTime')
...
def time_encoding(i='train'):
    df = pd.read_excel(f'./data/q1/lstm_{i}.xlsx')
    ...
    df.to_excel(f'./data/q1/lstm_{i}.xlsx')
