...
def packer(batch_size=64, sequence_length=32, feature_name_list=None, a=0.7, flag=0, need_scaler=0):
    if feature_name_list is None:
        feature_name_list = []
    if flag == 0:
        df = pd.read_excel('./data/q1/lstm_train.xlsx', index_col='DateTime')[feature_name_list]
    else:
        df = pd.read_excel('./data/q1/lstm_test.xlsx', index_col='DateTime')[feature_name_list]

    scaler = MinMaxScaler()
    features = df[['LC']]
    scaled_features = scaler.fit_transform(features)
    df[['LC']] = scaled_features
...
def packer_1(pred_ind, sequence_length=64, feature_name_list=None, a=0.7, need_scaler=0, whole_length=None):
    df = pd.read_excel('./data/q1/lstm_predict.xlsx', index_col='DateTime')[feature_name_list]
    feature_list = [i for i in range(len(feature_name_list))]
